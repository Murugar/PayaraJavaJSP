...
import com.iqmsoft.payara.demo.models.MemList;
...
public class GetServlet extends HttpServlet {
    private static final Logger log = Logger.getLogger(GetServlet.class.getCanonicalName());
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        Collection<MemEntity> membersWithName = bean.getTeamMemberByName(name);

        if (membersWithName.size() == 1) {
            MemEntity member = bean.getTeamMemberById(membersWithName.iterator().next().getId());
            ...
            MemList memberList = new MemList();

            String listItems = "";
            System.out.println(membersWithName.toString());
            for (MemEntity m : membersWithName) {
